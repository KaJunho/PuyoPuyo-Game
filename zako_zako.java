...
public class zako_zako extends AbstractSamplePlayer {
	...
	public Action doMyTurn() {
		...
		if(prep_period == 0 && attack_period == 0) {
			prep_period = 30;
		    attack_period = 0;
		}
		...
		//reinforce cycle
		if((enemy_total_puyo > 38 && haveOjama(enemy_field) == true) || getEnemyBoard().getTotalNumberOfOjama() != 0) {
			if(max_combo > 1) {
				if(max_combo > next_max_combo) {
					result = find_max_combo(current_map, current_field, current_puyo);
				    return new Action(PuyoDirection.values()[result[0]], result[1]);
			...
			result = preparation(current_map, current_field, current_puyo, next_puyo, nextnext_puyo);
		}
	...
	public int[] preparation(int [][] map, Field field, Puyo cur_puyo, Puyo next_puyo, Puyo nextnext_puyo) {
		...
		for(int d1 = 0; d1 < 4; d1++) {
			for(int col1 = 0; col1 < 6; col1++) {
				...
				add_map(map1, puyo_coord[0], puyo_coord[1], cur_puyo.getPuyoType(PuyoNumber.FIRST));
			    add_map(map1, puyo_coord[2], puyo_coord[3], cur_puyo.getPuyoType(PuyoNumber.SECOND));
			    neighbor1 = create_neighbor(map1);
			    
			    if(cur_puyo.getPuyoType(PuyoNumber.FIRST) != cur_puyo.getPuyoType(PuyoNumber.SECOND)) {
			    	int link_jiku = map_search(puyo_coord[0], puyo_coord[1], neighbor1, map1);
			    	int link_kumi = map_search(puyo_coord[2], puyo_coord[3], neighbor1, map1);
			    	if(link_jiku >= 4 || link_kumi >= 4) continue;
			    	else {
			    		/*
			    		if(col1 == 2 || col1 == 3 ) bonus = 1.2;
			    		else {
			    			if((col1 == 1 || col1 == 4) && (d1 != 1 && d1 != 3)) bonus = 1.2;
			    			else bonus = 1;
			    		}
			    		
		
			    		reward1 = (Math.pow(5, link_jiku) + Math.pow(5, link_kumi)) * bonus;
			    	}
			    }
			    else {
			    	int link = map_search(puyo_coord[0], puyo_coord[1], neighbor1, map1);
			    	if(link >= 4) continue;
			    	else {
			    		/*
			    		if(col1 == 2 || col1 == 3 ) bonus = 1.2;
			    ...
			    for(int d2 = 0; d2 < 4; d2++) {
			    	for(int col2 = 0; col2 < 6; col2++) {
						...
						int combo = return_combo(after_action1, dir2, col2, next_puyo);
						reward2 = reward1 + Math.pow(5, combo);
						if(reward2 >= max_reward) {
							max_reward = reward2;
							result[0] = d1;
							result[1] = col1;
						}
					...
					for(int col2 = 0; col2 < 6; col2++) {
						...
						for(int d3 = 0; d3 < 4; d3++) {
							for(int col3 = 0; col3 < 6; col3++) {
								...
								reward3 = (reward1 + reward2) * (combo+1);
